...
class CLIP_Adapter(nn.Module):
    def __init__(self, in_ch, mid_ch, out_ch, G_ch, CLIP_ch, cond_dim, k, s, p, map_num, CLIP):
        ...
        self.weight = nn.Parameter(torch.ones((8, CLIP_ch), requires_grad=True))
        self.alpha = nn.Parameter(torch.tensor(1.0), requires_grad=True)

    def add_adaptive_weight(self, prompts):
        weight = self.weight.unsqueeze(0)  # shape: (1, 8, CLIP_ch)
        weight = torch.softmax(weight, dim=1)  # shape: (1, 8, CLIP_ch)
        #weight = weight.squeeze(0)  # shape: (8, CLIP_ch)
        prompts = prompts.view(-1, 8, self.CLIP_ch)  # shape: (batch_size, 8, CLIP_ch)
        weight = weight.repeat(prompts.size(0),1,1)
        prompts = torch.mul(weight, prompts)
        #prompts = torch.sum(prompts, dim=1) #shape: (batch_size, CLIP_ch)
        return prompts
    
    def sequential_encoding(self, x, d_model, max_len):
        pos = torch.arange(0, d_model, dtype=torch.float).unsqueeze(1)
        div = torch.exp(torch.arange(0, max_len, 2).float() * (-math.log(10000.0) / d_model))
        pe = torch.zeros(1, d_model, max_len)
        pe[0, :, 0::2] = torch.sin(pos * div)
        pe[0, :, 1::2] = torch.cos(pos * div)
        pe = pe[:, :x.size(0), :].squeeze(0).to(x.device)
        return pe
        
       
    def forward(self, out, c, ingre):
        seq = self.sequential_encoding(ingre, self.CLIP_ch, ingre.size(1))
        ingre = seq + ingre
        prompts = self.fc_prompt(ingre).view(ingre.size(0),-1,self.CLIP_ch)
        prompts = self.add_adaptive_weight(prompts)
        for FBlock in self.FBlocks:
            out = FBlock(out,c)
        fuse_feat = self.conv_fuse(out)
        map_feat = self.CLIP_ViT(fuse_feat,prompts)
        return self.conv(fuse_feat+0.1* self.alpha * map_feat)
...
class NetG(nn.Module):
    ...
    def forward(self, noise, c, ingre, eval=False): # x=noise, c=ent_emb, ingre=ingredient embs
        with torch.cuda.amp.autocast() if self.mixed_precision and not eval else dummy_context_mgr() as mp:
            ...
            ingre = torch.cat((noise, ingre), dim=1)
            out = self.mapping(self.fc_code(noise).view(noise.size(0), self.code_ch, self.code_sz, self.code_sz), cond, ingre)
...
class IFBLK(nn.Module):
    def __init__(self, indim, outdim, device):
        super(IFBLK, self).__init__()
        self.device=device
    ...
    def contact(self,feature1, feature2):
        out_product = torch.mm(feature1, feature2.T).to(self.device)
        out_product = torch.sum(out_product,dim=1,keepdim=True)
        concated_feature = torch.cat([feature1, out_product, feature2], dim=-1)
        out_feature = self.fc(concated_feature)
        return out_feature
    
    def forward(self, x):
        x1=x[0]
        for num in range(len(x)-1):
            instruc1 = x1.to(self.device).to(torch.float32)
            instruc2 = x[num+1].to(self.device).to(torch.float32)
            x1=self.contact(instruc1,instruc2)
        return x1
            
class NetF(nn.Module):
    def __init__(self, indim, outdim, device, batch_size, mixed_precision):
        super(NetF, self).__init__()
        self.device = device
        self.batch_size = batch_size
        self.mixed_precision = mixed_precision
        self.IFBlock = IFBLK(indim, outdim, device)
        
    def forward(self, sent_embs, sent_instruc_embs, sent_ingre_embs):
        dim=sent_embs.shape[0]
        with torch.cuda.amp.autocast() if self.mixed_precision and not eval else dummy_context_mgr() as mp:
            FUSE = 'True'
            if FUSE == 'True':
                x1 = self.IFBlock(sent_instruc_embs).half()
            else:
                x1 = sent_instruc_embs.view([dim,-1]).to(torch.float16)
            dim=sent_embs.shape[0]
            weight = torch.randn([dim, 1], requires_grad=True).to(self.device).to(torch.float16)
            attention_scores = torch.matmul(sent_embs.T, weight)  # shape: [512, 1]
            attention_scores = torch.matmul(x1, attention_scores)  # shape: [batchsize, 1]
            attention_weights = torch.softmax(attention_scores, dim=-1)  # shape: [batchsize, 1]
            out = torch.matmul(attention_weights.unsqueeze(2), sent_embs.unsqueeze(1)).squeeze(1) + torch.matmul((1-attention_weights).unsqueeze(2), x1.unsqueeze(1)).squeeze(1)  # shape: [batchsize, 512]
        return out
