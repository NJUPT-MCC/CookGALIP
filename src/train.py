...
from lib.utils import load_models_opt,save_models_opt,save_models,load_npz
...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='CookGALIP',
                        help='the model for training')
...
def main(args):
    ...
    CLIP4trn, CLIP4evl, image_encoder, text_encoder, netG, netD, netC, IFBlock = prepare_models(args)
    fixed_img, fixed_sent, fixed_ingre, fixed_z = get_fix_data(train_dl, valid_dl, text_encoder, IFBlock, args)
    if (args.multi_gpus==True) and (get_rank() != 0):
        None
    else:
    ...
    optimizerF = torch.optim.Adam(IFBlock.parameters(), lr=0.0001, betas=(0.0, 0.9))
    if args.mixed_precision==True:
        ...
        scaler_F = torch.cuda.amp.GradScaler(growth_interval=args.growth_interval)
    else:
        scaler_D = None
        scaler_G = None
        scaler_F = None
    ...
    if args.state_epoch!=1:
        ...
        netG, netD, netC, IFBlock, optimizerG, optimizerD = load_models_opt(netG, netD, netC, IFBlock, optimizerG, optimizerD, path, args.multi_gpus)
    ...
    for epoch in range(start_epoch, args.max_epoch, 1):
        ...
        torch.cuda.empty_cache()
        train(train_dl, netG, netD, netC, IFBlock, text_encoder, image_encoder, optimizerG, optimizerD, optimizerF, scaler_G, scaler_D, scaler_F, args)
        torch.cuda.empty_cache()
        # save
        if epoch%save_interval==0:
            save_models_opt(netG, netD, netC, IFBlock, optimizerG, optimizerD, epoch, args.multi_gpus, args.model_save_file)
            torch.cuda.empty_cache()
        # sample
        if epoch%gen_interval==0:
            sample(fixed_z, fixed_sent, fixed_ingre, netG, args.multi_gpus, epoch, args.img_save_dir, writer, args.batch_size)
            torch.cuda.empty_cache()
        # test
        if epoch%test_interval==0:
            #torch.cuda.empty_cache()
            FID, TI_score,IS = test(valid_dl, text_encoder, netG, CLIP4evl, IFBlock, args.device, m1, s1, epoch, args.max_epoch, args.sample_times, args.z_dim, 2*args.batch_size)
            torch.cuda.empty_cache()
        if (args.multi_gpus==True) and (get_rank() != 0):
            None
        else:
            if epoch%test_interval==0:
                ...
                writer.add_scalar('IS', IS, epoch)
                print('The %d epoch FID: %.2f, CLIP_Score: %.2f, IS: %.2f' % (epoch,FID,TI_score*100,IS))
            end_t = time.time()
